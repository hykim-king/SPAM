...
<head>
    ...
    <style>
        ...
        .status-wait { background-color: #e9ecef; color: #495057; } /* 처리 대기 */
        .status-ing { background-color: #e3fafc; color: #0b7285; }  /* 접수/진행중 */
        .status-done { background-color: #ebfbee; color: #2b8a3e; } /* 처리 완료 */
...
<div class="detail-container">
        ...
        <div>
            <c:choose>
                <c:when test="${reportVO.reportStatus eq '01'}">
                    <span class="status-badge status-wait">처리 대기</span>
                </c:when>
                <c:when test="${reportVO.reportStatus eq '02'}">
                    <span class="status-badge status-ing">진행 중</span>
                </c:when>
                <c:when test="${reportVO.reportStatus eq '03'}">
                    <span class="status-badge status-done">처리 완료</span>
                </c:when>
                <c:otherwise>
    ...
    <table class="info-table">
        ...
        <tr>
            <th>신고자 회원번호</th>
            <td>${reportVO.reporterNo}</td>
        </tr>
        <tr>
            <th>피신고자 회원번호</th>
            <td>${reportVO.reportedUserNo != null ? reportVO.reportedUserNo : '없음 (대상물 신고)'}</td>
        </tr>
        <tr>
            <th>신고 대상 ID (PK)</th>
        ...
        <c:if test="${reportVO.reportStatus eq '03'}">
            <tr>
                <th>담당 관리자</th>
                <td>${reportVO.adminNo}번 관리자</td>
            </tr>
            <tr>
                <th>처리가 완료된 날짜</th>
