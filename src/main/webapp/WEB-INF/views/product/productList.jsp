    ...
    <main class="product-page product-list-page">
        ...
        <section class="product-filter-panel" aria-label="상품 검색 필터">
            ...
            <form class="product-filter-form" action="${CP}/product/list.do" method="get">
                ...
                <input type="hidden" name="includeSold" value="${search.includeSold}">
                ...
                <div class="product-filter-field product-price-field">
                    <label for="productPriceRange">가격대</label>
                    <select class="product-filter-select js-product-custom-select" id="productPriceRange" name="priceRange">
                        <option value="" ${empty search.priceRange ? 'selected' : ''}>전체 가격</option>
        ...
        <div class="product-list-layout">
            <aside class="product-category-sidebar" aria-label="상품 카테고리 필터">
                ...
                <nav class="product-category-list">
                    <c:url var="allCategoryUrl" value="/product/list.do">
                        ...
                        <c:param name="includeSold" value="${search.includeSold}" />
                        <c:param name="sort" value="${search.sort}" />
                        <c:param name="pageNo" value="1" />
                        <c:param name="pageSize" value="${search.pageSize}" />
                    </c:url>
                    <a class="${empty search.categoryNo ? 'is-active' : ''}" href="${allCategoryUrl}">전체</a>
            ...
            <section class="product-results" aria-labelledby="productResultTitle">
                <div class="product-results-toolbar">
                    ...
                    <form class="product-sort-form" action="${CP}/product/list.do" method="get">
                        ...
                        <input type="hidden" name="pageNo" value="1">
                        <input type="hidden" name="pageSize" value="${search.pageSize}">
                        <label class="product-sold-toggle" for="includeSold">
                            <input class="product-sold-toggle-input js-product-list-auto-submit" id="includeSold" type="checkbox"
                                   name="includeSold" value="true" ${search.includeSold ? 'checked' : ''}>
                            <span class="product-sold-toggle-switch" aria-hidden="true"></span>
                            <span>판매 완료 상품 포함</span>
                        </label>
                        <fieldset class="product-sort-toggle" aria-label="상품 정렬">
                            <legend class="product-visually-hidden">상품 정렬</legend>
                            <input class="product-toggle-input js-product-list-auto-submit" id="sortLatest" type="radio" name="sort" value="latest" ${empty search.sort or search.sort eq 'latest' ? 'checked' : ''}>
                            <label class="product-toggle-option" for="sortLatest">최신순</label>
                            <input class="product-toggle-input js-product-list-auto-submit" id="sortPopular" type="radio" name="sort" value="popular" ${search.sort eq 'popular' ? 'checked' : ''}>
                            <label class="product-toggle-option" for="sortPopular">인기순</label>
                            <input class="product-toggle-input js-product-list-auto-submit" id="sortPriceLow" type="radio" name="sort" value="priceLow" ${search.sort eq 'priceLow' ? 'checked' : ''}>
                            <label class="product-toggle-option" for="sortPriceLow">낮은 금액순</label>
                            <input class="product-toggle-input js-product-list-auto-submit" id="sortPriceHigh" type="radio" name="sort" value="priceHigh" ${search.sort eq 'priceHigh' ? 'checked' : ''}>
                            <label class="product-toggle-option" for="sortPriceHigh">높은 금액순</label>
                        </fieldset>
                    </form>
                </div>
